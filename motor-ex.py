...
It's a very powerful tool to create a customized function with pre-defined or new
arguments.
'''
...
THROTTLE_SPEED = {0: 0, 1: 0.35, 2: 0.5, 3: 0.7, 4: 0.9} # dictionary of speeds;
0: stop, 4: max speed
...
def set_throttle(motor_name, speed, factor=1):
    ...
    - factor: 1 or -1 showing forward or backward motions - the default is
    forward (1)
    Output:
    - applies current to the dc motors wrt the SyncForwardR or SyncBackwardR
    hyper params
    """
    # If the motor_name is 'R' then, we have to apply sync coefficients - depending
    on the direction:
        ?
...
def move(duration=0.3, speed=2, factor_r=1, factor_l=1):
    ...
    - speed: the motor speed from THROTTLE_SPEED dictionary, default: speed 2
    (50% of max throttle)
    - factor_r, factor_l: 1 or -1 : show which direction the motor should
    rotate
    Output:
    - the motors' motion
    """
